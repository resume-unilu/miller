...
class StoryViewSet(viewsets.ModelViewSet):
  ...
  def _getUserAuthorizations(self, request):
    if request.user.is_staff:
      q = Story.objects.all()
    elif request.user.groups.filter(name='chief-reviewers').exists():
      q = Story.objects.filter(Q(status=Story.PUBLIC) | Q(status=Story.PENDING) | Q(status=Story.EDITING) | Q(status=Story.REVIEW)).distinct()
